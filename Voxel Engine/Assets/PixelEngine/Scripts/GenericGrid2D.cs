...
namespace TheAshBot.PixelEngine
{
    public class GenericGrid2D<TGridObject> : Grid2D
    {
        ...
        public GenericGrid2D(byte width, byte height, float cellSize, Vector2 originPosition, Func<GenericGrid2D<TGridObject>, byte, byte, TGridObject> createGridObject, bool showDebug, Transform parent)
        ...
        /// <param name="originPosition">This is the position of the bottum left grid object(AKA the origin)</param>
        /// <param name="createGridObject">This is the the value that all the gid object will default to<code>(GenericGrid grid, int x, int y) => new TGridObject(grid, x, y)</code></param>
        public GenericGrid2D(byte width, byte height, float cellSize, Vector2 originPosition, Func<GenericGrid2D<TGridObject>, byte, byte, TGridObject> createGridObject)
        ...
        /// <param name="y"></param>
        /// <param name="value"></param>
        public void SetGridObjectWithoutNotifying(int x, int y, TGridObject value)
        {
            if (x >= 0 && y >= 0 && x < width && y < height)
            {
                if (value == null)
        ...
        /// <param name="worldPosition">This is the grid objects world position<</param>
        /// <param name="value">This is the value it is being set to</param>
        public void SetGridObjectWithoutNotifying(Vector2 worldPosition, TGridObject value)
        {
            GetXY(worldPosition, out int x, out int y);
            SetGridObjectWithoutNotifying(x, y, value);
        }
